...
Future<String> generateRandomDailyAffirmationFromOpenAI() async {
  final url = Uri.parse('https://api.openai.com/v1/chat/completions');
  final response = await http.post(
    url,
    headers: {
      ...
      'messages': [
        {
          'role': 'system',
          'content':
              'You are a helpful assistant that provides daily affirmation for people.'
        },
        {'role': 'user', 'content': generateRandomDailyAffirmation()}
      ],
      'max_tokens': 200,
    }),
  );
